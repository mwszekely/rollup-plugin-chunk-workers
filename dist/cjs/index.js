        ...
        async transform(code, id) {
            if (filter(id)) {
                try {
                    ...
                    acornWalk.simple(moduleInfo.ast, {
                        "CallExpression": (node) => {
                            const exp = node;
                    ...
                    filesToEmit.forEach(({ url, urlStart, urlEnd }) => {
                        const fileId = this.emitFile({
                            type: "chunk",
                            id: url,
                            fileName: path.relative(projectDir, url),
                            importer: id
                        });
                        const fileName = this.getFileName(fileId);
                        if (urlStart && urlEnd)
                            magicString.update(urlStart, urlEnd, JSON.stringify(pluginutils.normalizePath(fileName)));
                    });
                    return {
